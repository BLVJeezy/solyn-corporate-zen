...
import { Phone, Mail, Globe, Upload, FileText, Download, Trash2, Pencil, ExternalLink } from "lucide-react";
...
export default function MobileClientDrawer({ client, open, onClose, onEdit }: Props) {
  const { data: invoices = [], isLoading: invoicesLoading } = useClientInvoices(client?.id ?? null);
  ...
  const handleDeleteInvoice = (id: string, filePath: string) => {
    ...
    <Drawer open={open} onOpenChange={(o) => !o && onClose()}>
      <DrawerContent className="bg-card border-border max-h-[90vh]">
        <DrawerHeader className="pb-2">
          <DrawerTitle className="text-card-foreground flex items-center justify-between">
            <span className="truncate">{client.company || client.name}</span>
            <Button variant="ghost" size="icon" className="h-8 w-8 shrink-0" onClick={() => onEdit(client)}>
        ...
        <div className="px-4 pb-6 space-y-4 overflow-y-auto">
          ...
          <div className="grid grid-cols-3 gap-2">
            {client.phone && (
              <a href={`tel:${client.phone}`} className="flex flex-col items-center gap-1 bg-muted rounded-lg p-3 active:bg-muted/70">
                <Phone className="w-5 h-5 text-primary" />
                <span className="text-[10px] text-muted-foreground">Bellen</span>
              </a>
            )}
            {client.email && (
              <a href={`mailto:${client.email}`} className="flex flex-col items-center gap-1 bg-muted rounded-lg p-3 active:bg-muted/70">
                <Mail className="w-5 h-5 text-primary" />
                <span className="text-[10px] text-muted-foreground">E-mail</span>
              </a>
            )}
            {websiteUrl && (
              <a href={websiteUrl} target="_blank" rel="noopener noreferrer" className="flex flex-col items-center gap-1 bg-muted rounded-lg p-3 active:bg-muted/70">
              ...
              <div className="pt-2 border-t border-border space-y-1.5">
                ...
                {creditCost > 0 && (
                  <div className="flex justify-between text-xs">
                    ...
                    <span className="text-red-500 font-semibold">-€{creditCost.toFixed(2)}</span>
                  </div>
                )}
                <div className="flex justify-between text-xs">
                  <span className="text-muted-foreground">Tx fees (2.8%)</span>
                  <span className="text-red-500 font-semibold">-€{txFees.toFixed(2)}</span>
                </div>
          ...
          <div className="space-y-2 text-sm">
            {client.start_date && (
              <div className="flex justify-between">
                ...
                <span className="text-card-foreground text-xs">{format(new Date(client.start_date), "d MMM yyyy", { locale: nl })}</span>
              </div>
            )}
            {client.notes && (
              <div className="pt-2 border-t border-border">
                <span className="text-muted-foreground text-xs block mb-1">Notities</span>
                <p className="text-card-foreground text-xs whitespace-pre-wrap">{client.notes}</p>
              </div>
            )}
          </div>
          ...
          <div className="pt-2 border-t border-border">
            <div className="flex items-center justify-between mb-2">
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Facturen</p>
              <Button
                variant="outline"
                size="sm"
              ...
              >
                ...
                {uploadInvoice.isPending ? "..." : "Upload"}
              </Button>
              <input ref={fileInputRef} type="file" accept=".pdf,.png,.jpg,.jpeg,.doc,.docx" className="hidden" onChange={handleFileChange} />
            </div>

            {invoicesLoading ? (
              <p className="text-muted-foreground text-xs text-center py-3">Laden...</p>
            ) : invoices.length === 0 ? (
              <p className="text-muted-foreground text-xs text-center py-3">Nog geen facturen</p>
            ) : (
                  ...
                  <div key={inv.id} className="flex items-center justify-between bg-muted rounded-md px-3 py-2">
                    <div className="flex items-center gap-2 min-w-0">
                      ...
                      <div className="min-w-0">
                        <p className="text-xs text-card-foreground truncate">{inv.file_name}</p>
                    ...
                    <div className="flex gap-0.5 shrink-0">
                      <Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground hover:text-primary" onClick={() => handleDownload(inv.file_path)}>
                        <Download className="w-3.5 h-3.5" />
                      </Button>
                      <Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground hover:text-destructive" onClick={() => handleDeleteInvoice(inv.id, inv.file_path)}>
