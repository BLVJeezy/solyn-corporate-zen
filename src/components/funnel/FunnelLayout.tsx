...
const FunnelLayout = ({ step, totalSteps, children, hideProgress }: FunnelLayoutProps) => {
  ...
  const stepLabel = t("funnel.step").replace("{n}", String(step)).replace("{m}", String(totalSteps));
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0f] text-white relative overflow-hidden">
      ...
      <header className="relative z-10 px-6 lg:px-10 py-6 flex items-center justify-between">
        ...
        {!hideProgress && (
          <span className="text-xs text-white/40 font-medium tabular-nums">{stepLabel}</span>
        )}
      </header>
